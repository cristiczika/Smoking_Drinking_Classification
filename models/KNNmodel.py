...
class SmokingDrinkingKNNTrainer:
    ...
    def train(self):
        ...
        # GridSearch pe un subset mai mic, ca la Decision Tree
        ...
        base_model = MultiOutputClassifier(KNeighborsClassifier())
        ...
        joblib.dump(self.best_model, os.path.join(self.results_folder, 'knn_model.joblib'))
        joblib.dump(search.best_params_, os.path.join(self.results_folder, 'knn_best_params.joblib'))

        with open(os.path.join(self.results_folder, "knn_metrics.txt"), "w") as f:
            f.write("KNN - Multi-Label Evaluation\n")
            f.write("=" * 50 + "\n\n")
...
if __name__ == "__main__":
    ...
    trainer = SmokingDrinkingKNNTrainer(data_folder=data_dir, results_folder=models_dir)
